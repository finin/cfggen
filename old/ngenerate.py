...
def rewrite(g, tokens, into):
    """Replace tokens in list with a random entry in grammar g (recursively)."""
    def rw(t):
        return 
    for token in tokens:
        if token in g:
...
def generate_tree(g, start='S'):
    """Use grammar g to rewrite start returning a tree"""
...
if __name__ == "__main__":
    ...
    p = ArgumentParser(description='Generate a random string from a grammar')
    ...
    g = load_grammar(args.gfile)
    if args.t:
        pprint(generate_tree(g, args.start))
    else:
        print generate(g, args.start)
